...
{
	const string inputObjectName[] = {"flipphones", "forks", "hammers", "mugs", "pliers", "scissors", "watches"};
	string filename;
	CImg<unsigned char> img1;
	double totalTime = 0.0f;
	for (int objectIndex = 0; objectIndex < 7; objectIndex++) {
		for (int imageIndex = 0; imageIndex < 28; imageIndex++) {
			...
			for (i = 0; i < 3; ++i) {
				cluster[i] = new int[height*width];
			}

			int **unk_points = new int*[height*width];
			for (i = 0; i < height * width; ++i) {
				unk_points[i] = new int[2];
			}

			int **fg_points = new int*[height*width];
			for (i = 0; i < height * width; ++i) {
				fg_points[i] = new int[2];
			}
			int **bg_points = new int*[height*width];
			for (i = 0; i < height * width; ++i) {
				bg_points[i] = new int[2];;
			}
